...
async function scrape() {
  const base = 'https://dicoha.com.co'
  const results = []
  ...
  try {
    ...
    // Busscar tarjetas de producto en la home / categorías (esto es heurístico)
    const cards = await page.$$(
      'article, .product, .woocommerce-LoopProduct-link, .producto, .card'
    )
    ...
    for (let i = 0; i < Math.min(cards.length, 12); i++) {
      try {
        ...
        if (name || price) {
          results.push({ name: name || 'sin-nombre', price: price || 'sin-precio', url, source: 'dicoha' })
        }
      } catch (e) {
        // ignora tarjeta
      }
